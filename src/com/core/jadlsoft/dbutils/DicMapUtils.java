package com.core.jadlsoft.dbutils;
...
public class DicMapUtils implements RowMapper {
	...
	private String[] tableList;		//需要缓存的数据表信息列表
	private String keyColumn;		//字典表中的主键列
	
	private String dicConfigFile;	//字典配置文件
	...
	public void setDicConfigFile(String dicConfigFile) {
		this.dicConfigFile = dicConfigFile;
	}

	public void setCache() {
//		dicMapCacheForTranslate = new HashMap();
		...
		//dicMapCache.put("permiss", jdbcTemplate.query("SELECT * from t_xt_module  order by sxh",
	...
	public void cacheOneTable( RowMapper rowmapper, String table) {
		...
		if (diccfgs.length > 2) {
			...
			sql = "" + diccfgs[1] + " T";
			keycol = diccfgs[2];
			order = " ORDER BY T." + diccfgs[3];
		} else {
			tableName = diccfgs[0];
			sql = tableName;
			keycol = keyColumn;
		}
		if (!order.equalsIgnoreCase(" ") && !order.equalsIgnoreCase("null")) {
			sql = "SELECT * FROM " + sql + order;
		} else {
			sql = "SELECT * FROM " + sql;
		}
		List datalist = jdbcTemplate.query(sql, rowmapper);
	...
	public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		for (int i = 1; i <= columnCount; i++) {
			...
			if(columnType.equalsIgnoreCase("DATE")){
				// 如果是DATE类型，格式化并精确到秒
				String value = DateUtils.getDatatoString(rs.getTimestamp(i), "yyyy-MM-dd HH:mm:ss");
