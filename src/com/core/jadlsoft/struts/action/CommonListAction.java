...
import com.core.jadlsoft.business.CommonListManager;
import com.core.jadlsoft.dbutils.DicMapUtils;
import com.core.jadlsoft.dbutils.SQLHelper;
	...
	protected String doGetList() {
		...
		if(forwardMap.containsKey(realServletPath)) {
			...
			try {
				...
				data = listManager.getDataCountAndList(null, condList,super.getUserSessionBean(), (String) forwardinfo.get("datasource"), (String) 
forwardinfo.get("class"),(String) forwardinfo.get("afterClass"), pagesize, pageno);
			} catch(Exception e) {
	...
	private boolean setConditions(String condition) {
		UserSessionBean userBean = (UserSessionBean)getUserSessionBean();
		...
		if(condition.startsWith("&&")) {	
			...
			for(int i=0;i<conds.length/3;i++) {
				//默认条件传pagesize~=~30
				if(conds[3 * i].equalsIgnoreCase("pageSize")){
					pagesize = Integer.valueOf(conds[3 * i + 2]);
					continue;
				}
				condList.add(conds[3 * i] + "~" + conds[3 * i + 1] + "~"
						+ conds[3 * i + 2].replaceAll("\\Q[$now]\\E",DateUtils.getCurrentData()).replaceAll("\\Q[$xzqh]\\E", userBean.getXzqh())
						.replaceAll("\\Q[$qydm]\\E", userBean.getQydm()));
	...
	protected String doSaveList() {
		...
		if (!"savelist".equals(choice)) {
			...
			int count = listManager.getCount(condList, (String) forwardinfo.get("datasource"),super.getUserSessionBean(), (String) forwardinfo.get
...
("class"));
			...
			if (count <= 5000) {
				...
				try {
					...
					data = listManager.getDataCountAndList(null, condList,super.getUserSessionBean(),  (String) forwardinfo.get("datasource"), 
